...
# Function to get content for each column using GPT-4
def extract_content_with_gpt4(text, prompt):
    response = client.chat.create(
        model=deployment_name,
        messages=[
            {"role": "system", "content": "你是一个北京信诺时代科技发展有限公司的合同关键信息提取助手，用户将询问有关签约客户的合同关键信息，你需要在与不同客户签署的买卖合同和MSP服务协议中，提取出如下的关键信息。"},
...
if uploaded_file is not None:
    ...
    if "买卖合同" in full_text or "服务合同" in full_text:
        ...
        for key, prompt in sales_contract_prompts.items():
            extracted_info_sales[key] = extract_content_with_gpt4(full_text, prompt)
        df = pd.DataFrame([extracted_info_sales])

    elif "MSP服务协议" in full_text:
        st.write("检测到MSP服务协议")
        for key, prompt in msp_agreement_prompts.items():
            extracted_info_msp[key] = extract_content_with_gpt4(full_text, prompt)
